...
namespace spoa
{
  ...
  rust::Vec<AlignmentPair> alignment_to_vec(const spoa::Alignment &alignment)
  {
    rust::Vec<AlignmentPair> result;
    result.reserve(alignment.size());

    for (const auto &pair : alignment)
    {
      result.push_back(AlignmentPair{pair.first, pair.second});
    }
